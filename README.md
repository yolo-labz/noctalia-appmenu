...
| Firefox / Thunderbird | Fallback (full menu via flag) | No native menubar by default → [desktop fallback](#app-menu-fallback). Set `accessibility.force_disabled = 0` (NixOS: in `programs.firefox…settings`, not `about:config` — `user.js` is HM-managed), restart the browser, **then restart the bridge** (`systemctl --user restart noctalia-appmenu-bridge.service`) — see the Firefox note under [Caveats](#caveats). |
...
- **Firefox / Thunderbird.** A native menubar surfaces only when `accessibility.force_disabled = 0` is set and the browser is restarted; otherwise the [desktop fallback](#app-menu-fallback) applies. Firefox exposes the full menubar over AT-SPI even while its visual Menu Bar is autohidden (`frame → tool bar "Menu Bar" → menu bar → File/Edit/View/…`); top-level menus realise their items lazily on first open (the self-heal cascade fills them). Two gotchas:
  ...
  - **Restart the bridge after enabling.** If Firefox ran with a11y disabled while the bridge was already up, the bridge has *learned* to skip it (a not-on-bus app drains the fetch budget, so the skip is permanent for the bridge process's life). Flipping the pref + restarting Firefox is not enough — also `systemctl --user restart noctalia-appmenu-bridge.service` so the learned-skip set is cleared. Symptom if you forget: `active.json` stays `source:"desktop-fallback"` for Firefox even though `force_disabled=0`.
